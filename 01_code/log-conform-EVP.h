...
const scalar zerocc[] = 0.00001;
(const) scalar trA = zeroc;
scalar solidreg;
double solidthresh=0.001;
...
event init (i = 0) {
#if AXI
  boundary((scalar *){tau_p, mytaup, tau_qq, mytauqq});
#else
  boundary((scalar *){tau_p, mytaup});
#endif
}
...
//      if (solidreg[]==0.5) 
//      printf("what happens here, %g %g %g %g\n",x,y,solidreg[], lambda[]);
...
//EVP exponential version
	...
	tau_p.x.x[] = fa*(nu*A.x.x - 1.);
      ...
      tttt[] = tau_p.y.y[]+0.0;
      ttttt[] = tau_qq[]+0.0;
//     printf("%g %g %g %g %g\n",x,y,taupxx[],taupxy[],taupyy[]);
    }
  }

#if AXI
  boundary((scalar *){tau_p, tau_qq, mytaup, mytauqq});
#else
  boundary((scalar *){tau_p, mytaup});
#endif
}
